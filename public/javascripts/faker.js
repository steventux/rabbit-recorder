var Faker = (function(){
  ...
  return {
     ...
     sentence : function(len) {
       len = len || 10;
       return this.paragraph(len);
     },
     word : function() {
       return this.paragraph(1);
     },
     firstName : function() {
     ...
     fullName : function(separator) {
       separator = separator || " ";
       return [this.firstName(), this.lastName()].join(separator);
     },
     email : function() {
       return [this.fullName(".").toLowerCase(), DOMAINS[randomIndex(DOMAINS)]].join("@");
     } 
  }
